				...
				u16 offset, u8 *buf, u32 count)
{
	...
	case UART_TX:
...
#else
        {
            ...
            if( prxtx->rxcnt  && !prxtx->rxfifo ) {
                pr_err("rxcnt is %d, but fifo is NULL\n",prxtx->rxcnt);
                prxtx->rxcnt   = 0;
            }
            pr_info("===2\n");
            if(prxtx->rxcnt == 0){
