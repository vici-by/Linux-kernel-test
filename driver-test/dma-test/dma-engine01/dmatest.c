...
static bool dmatest_match_channel(struct dmatest_params *params,
		struct dma_chan *chan)
{
    pr_info("dma chan name is %s, channel name is %s\n", dma_chan_name(chan), params->channel);
...
{
    pr_info("dma dev name is %s, device name is %s\n", dev_name(device->dev), params->device);
...
static int dmatest_func(void *data)
{
    ...
    pr_info("dma %s,Support %u count ",dma_chan_name(chan),dev->chancnt);
    if (dma_has_cap(DMA_MEMCPY, dev->cap_mask)) {
        pr_info("\tSupport DMA_MEMCPY");
    }
    if (dma_has_cap(DMA_MEMSET, dev->cap_mask)) {
        pr_info("\tSupport DMA_MEMSET");
    }
    pr_info("dma dev name is %s,config is %pK,tx status %pK\n",dev->dev->init_name,dev->device_config,dev->device_tx_status);
	...
	while (!kthread_should_stop()
        ...
        printk("=======>Start device_prep_dma_memcpy\n");
		if (thread->type == DMA_MEMCPY)
