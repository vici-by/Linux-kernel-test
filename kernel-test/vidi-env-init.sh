...
do_x86_env()
{
	...
	then
		...
		version=v${version}
		cp -r ${LINUXDIR}  ./${LINUX} && cd ${LINUX} &&	git checkout  ${version} && git checkout  -b ${version} && cd ../  && ln -s ${x8664dir}/${LINUX} ${x8664dir}/linux
...
do_arm64_env()
{
	...
	then
		...
		version=$(echo "$LINUX" | cut -d "-" -f 2)
		version=v${version}
		cp -r ${LINUXDIR}  ./${LINUX} && cd ${LINUX} &&	git checkout  ${version} && git checkout  -b ${version} && cd ../ && ln -s ${arm64dir}/${LINUX} ${arm64dir}/linux
		# cp build_linux.sh linux
		# cp build_linux.sh linux  && cd linux && ./build_linux.sh init
		cd ${arm64dir}
	fi
